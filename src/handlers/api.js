...
export const apiGetSinglePrice = (asset = "", native = "USD") => {
  return cryptocompare.get(
    `/price?fsym=${asset}&tsyms=${native}&apiKey=${process.env.REACT_APP_CRYPTOCOMPARE_API_KEY}`
  );
};
...
export const apiGetHistoricalPrices = (
  ...
  return cryptocompare.get(
    `/pricehistorical?fsym=${assetSymbol}&tsyms=${nativeQuery}&ts=${timestamp}&apiKey=${process.env.REACT_APP_CRYPTOCOMPARE_API_KEY}`
  );
};
